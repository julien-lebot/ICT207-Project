...
Phoenix::tr1::shared_ptr<unsigned char> TexturedPolygons::LoadRawImageFile(char* filename, int width, int height)
{
	FILE* file;
	Phoenix::tr1::shared_ptr<unsigned char> image;
	// create memory space w x h x 3 (3 stores RGB values)
	//image = (unsigned char*)malloc(sizeof(unsigned char) * width * height * 3);
	image = Phoenix::tr1::shared_ptr<unsigned char>(new unsigned char[width * height * 3]);
	...
	fread(image.get(), width * height * 3, 1, file);
	fclose(file);
    return image;
}
