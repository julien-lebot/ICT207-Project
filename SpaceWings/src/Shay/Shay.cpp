...
void ShayWorld::myinit()
{
	...
	cam.Position(4000.0, 10450.0, 21000.0, 270.0);
...
void ShayWorld::CreateBoundingBoxes()
{
	// chanc block
	...
	cam.SetAABBMinX(12, -5000.0);
	cam.SetAABBMaxZ(12, 20240.0);
	cam.SetAABBMinZ(12, 0.0);

	// Lower canteen block
	cam.SetAABBMaxX(13, 2608.0);
	cam.SetAABBMinX(13, -5000.0);
	cam.SetAABBMaxZ(13, 49046.0);
	cam.SetAABBMinZ(13, 21000.0);
...
{	
	...
	cam.SetPlains (FLAT_PLAIN, 2608.0, 6500.0, 10450.0, 10450.0, 17000.0, 40000.0);
	...
	for (int i = 0; i < 18 ; i ++)
	{
		cam.SetPlains (FLAT_PLAIN, 31582.0, 33835, step, step, stepLength, stepLength + 142.0);
		step -= 48.0;
		stepLength -= 142.0;
		if ((i+3) % 5 == 0) 
		{
	...
	for (int i = 0; i < 18 ; i ++)
	{
		cam.SetPlains (FLAT_PLAIN, stepLength, stepLength + 142.0, step, step, 20240.0, 21000.0);		
		step -= 48.0;
		stepLength -= 142.0;
	}

	//corridoor floor
	cam.SetPlains (FLAT_PLAIN, 0.0, 2608.0, 9586.0, 9586.0, 20240.0, 21000.0);
}

//--------------------------------------------------------------------------------------
//  Delete raw image and clear memory
//--------------------------------------------------------------------------------------
void ShayWorld::DeleteImageFromMemory(unsigned char* tempImage)
{
	tempImage = NULL;
	if (tempImage == NULL)
	{
...
void ShayWorld::CreateTextures()
{
	...
	// load and create textures
	tp.CreateTexture(ABOVE_CHANC_TEXT, ImagePtr(tp.LoadTexture("data/abovechanctext.raw", 128, 1024)), 128, 1024);
	tp.CreateTexture(ABOVE_CHANC_TEXT_2, ImagePtr(tp.LoadTexture("data/abovechanctext2.raw", 128, 1024)), 128, 1024);
	tp.CreateTexture(ABOVE_CHANC_TEXT_3, ImagePtr(tp.LoadTexture("data/abovechanctext3.raw", 128, 1024)), 128, 1024);
	tp.CreateTexture(ABOVE_LIB_TEXT, ImagePtr(tp.LoadTexture("data/aboveLibText.raw", 1024, 256)), 1024, 256);
	tp.CreateTexture(ABOVE_LIB_TEXT_2, ImagePtr(tp.LoadTexture("data/aboveLibraryText2.raw", 1024, 256)), 1024, 256);
	tp.CreateTexture(ABOVE_UNDER_4B, ImagePtr(tp.LoadTexture("data/aboveUnder4B.raw",256, 128)), 256, 128);	
	tp.CreateTexture(ABOVE_WINDOW_BLOCK, ImagePtr(tp.LoadTexture("data/aboveWindows.raw", 128, 256)), 128, 256);
	tp.CreateTexture(ABOVE_WINDOW_BLOCK_2, ImagePtr(tp.LoadTexture("data/aboveWindows2.raw", 128, 256)), 128, 256);	
	tp.CreateTexture(ABOVE_WINDOW_BLOCK_CHANC, ImagePtr(tp.LoadTexture("data/aboveWindowsChanc.raw", 128, 256)), 128, 256);	
	tp.CreateTexture(ABOVE_UNDER_POSTS, ImagePtr(tp.LoadTexture("data/aboveWindows2posts.raw", 128, 256)), 128, 256);
	tp.CreateTexture(ABOVE_UNDER_POSTS_2, ImagePtr(tp.LoadTexture("data/aboveWindows2posts2.raw", 128, 256)), 256, 128);	
	tp.CreateTexture(ABOVE_LIB, ImagePtr(tp.LoadTexture("data/aboveWindowsLib.raw", 256, 256)), 256, 256);
	tp.CreateTexture(ABOVE_WINDOW_UNDER_LIB, ImagePtr(tp.LoadTexture("data/aboveWindows2lib.raw", 256, 128)), 256, 128);
	tp.CreateTexture(ABOVE_WINDOW_BLOCK_3, ImagePtr(tp.LoadTexture("data/aboveWindows3bricks.raw", 256, 256)), 256, 256);	
	tp.CreateTexture(ABOVE_PHYS_SCI_TEXT, ImagePtr(tp.LoadTexture("data/abovephysscitext.raw", 256, 1024)), 256, 1024);	
	tp.CreateTexture(ABOVE_WINDOW_BLOCK_XY_3, ImagePtr(tp.LoadTexture("data/aboveWindows3bricksXY.raw", 256, 256)), 256, 256);	
	tp.CreateTexture(ABOVE_WINDOW_EDGE_3B, ImagePtr(tp.LoadTexture("data/aboveWindowEdge.raw", 128, 256)), 128, 256);	
	tp.CreateTexture(ABOVE_WINDOW_EDGE_3B_LIB, ImagePtr(tp.LoadTexture("data/aboveWindowEdgeLib.raw", 256, 64)), 256, 64);	
	tp.CreateTexture(ABOVE_WINDOW_EDGE_4B_LIB, ImagePtr(tp.LoadTexture("data/aboveWindowsLibEdge.raw", 256, 64)), 256, 64);	
	tp.CreateTexture(ABOVE_TICKETS_TEXT, ImagePtr(tp.LoadTexture("data/aboveTicketsText.raw", 256, 256)), 256, 256);	
	tp.CreateTexture(ABOVE_CHANC_EDGE, ImagePtr(tp.LoadTexture("data/aboveWindowsEdge.raw", 128, 128)), 128, 128);	
	tp.CreateTexture(BELOW_ROOF_FILL, ImagePtr(tp.LoadTexture("data/belowrooffill.raw", 128, 256)), 128, 256);	
	tp.CreateTexture(BENCH_TOP, ImagePtr(tp.LoadTexture("data/bench.raw", 64, 64)), 64, 64);	
	tp.CreateTexture(BENCH_EDGE_SIDE, ImagePtr(tp.LoadTexture("data/benchedgeside.raw", 64, 64)), 64, 64);	
	tp.CreateTexture(BENCH_EDGE, ImagePtr(tp.LoadTexture("data/benchedge.raw", 64, 64)), 64, 64);	
	tp.CreateTexture(BENCH_EDGE_TOP, ImagePtr(tp.LoadTexture("data/benchedgetop.raw", 64, 64)), 64, 64);	
	tp.CreateTexture(BENCH_EDGE_2, ImagePtr(tp.LoadTexture("data/benchedge2.raw", 64, 64)), 64, 64);	
	tp.CreateTexture(BENCH_EDGE_3, ImagePtr(tp.LoadTexture("data/benchedge3.raw", 64, 64)), 64, 64);	
	tp.CreateTexture(BENCH_EDGE_TOP_2, ImagePtr(tp.LoadTexture("data/benchedgetop2.raw", 64, 64)), 64, 64);	
	tp.CreateTexture(BENCH_SIDE, ImagePtr(tp.LoadTexture("data/benchside.raw", 64, 64)), 64, 64);	
	tp.CreateTexture(BENCH_SIDE_2, ImagePtr(tp.LoadTexture("data/benchside2.raw", 64, 64)), 64, 64);	
	tp.CreateTexture(WALL_BRICK_YZ, ImagePtr(tp.LoadTexture("data/bricks1.raw", 128, 128)), 128, 128);	
	tp.CreateTexture(WALL_BRICK_XY, ImagePtr(tp.LoadTexture("data/bricks2.raw", 128, 128)), 128, 128);	
	tp.CreateTexture(WALL_BRICK_XY_END, ImagePtr(tp.LoadTexture("data/bricks2edge.raw", 128, 128)), 128, 128);	
	tp.CreateTexture(WALL_BRICK_YZ_END, ImagePtr(tp.LoadTexture("data/bricks1edge.raw", 128, 128)), 128, 128);	
	tp.CreateTexture(WALL_BRICK_3_4, ImagePtr(tp.LoadTexture("data/bricks075.raw", 128, 128)), 128, 128);	
	tp.CreateTexture(WALL_BRICK_XY_87WIDTH, ImagePtr(tp.LoadTexture("data/brick87.raw", 128, 128)), 128, 128);	
	tp.CreateTexture(WALL_BRICK_GAP_YZ, ImagePtr(tp.LoadTexture("data/brickgap.raw", 128, 32)), 128, 32);	
	tp.CreateTexture(WALL_BRICK_GAP2_YZ, ImagePtr(tp.LoadTexture("data/brickgap2.raw", 128, 32)), 128, 32);
	tp.CreateTexture(WALL_BRICK_SEC_SIGN, ImagePtr(tp.LoadTexture("data/bricksecsign.raw", 256, 128)), 256, 128);	
	tp.CreateTexture(SHADOW_BRICK, ImagePtr(tp.LoadTexture("data/brickshadow.raw", 128, 128)), 128, 128);	
	tp.CreateTexture(WALL_BRICK_USD_YZ, ImagePtr(tp.LoadTexture("data/bricksUSD.raw", 128, 128)), 128, 128);	
	tp.CreateTexture(CARPET, ImagePtr(tp.LoadTexture("data/carpet.raw", 32, 32)), 32, 32);	
	tp.CreateTexture(COFFEE_MACHINE, ImagePtr(tp.LoadTexture("data/coffeeMachine.raw", 128, 256)), 128, 256);	
	tp.CreateTexture(COKE_MACHINE, ImagePtr(tp.LoadTexture("data/cokeMachine.raw", 128, 256)), 128, 256);	
	tp.CreateTexture(CO_SIGN, ImagePtr(tp.LoadTexture("data/COsign.raw", 256, 128)), 256, 128);	
	tp.CreateTexture(COUNTER_SIDE, ImagePtr(tp.LoadTexture("data/counterside.raw", 4, 16)), 4, 16);	
	tp.CreateTexture(COUNTER_TOP, ImagePtr(tp.LoadTexture("data/countertop.raw", 1, 1)), 1, 1);	
	tp.CreateTexture(DRAINPIPE, ImagePtr(tp.LoadTexture("data/drainpipe.raw", 32, 2)), 32, 2);	
	tp.CreateTexture(DRINKS_EDGE, ImagePtr(tp.LoadTexture("data/drinksEdge.raw", 16, 2)), 16, 2);	
	tp.CreateTexture(DRINKS_SIDE, ImagePtr(tp.LoadTexture("data/drinksSide.raw", 64, 64)), 64, 64);	
	tp.CreateTexture(DRINKS_SIDE_2, ImagePtr(tp.LoadTexture("data/drinksSide2.raw", 64, 64)), 64, 64);
	tp.CreateTexture(DRINKS_TOP, ImagePtr(tp.LoadTexture("data/drinksSidetop.raw", 64, 64)), 64, 64);	
	tp.CreateTexture(DOORPAVE_1, ImagePtr(tp.LoadTexture("data/doorpave1.raw",  128, 256)), 128, 256);	
	tp.CreateTexture(DOOR_POST_CHANC, ImagePtr(tp.LoadTexture("data/doorpost1.raw", 1024, 128)), 1024, 128);	
	tp.CreateTexture(DOOR_POST_SECURITY, ImagePtr(tp.LoadTexture("data/doorpostSec.raw", 1024, 128)), 1024, 128);	
	tp.CreateTexture(DOOR_SIDEPOST_CHANC, ImagePtr(tp.LoadTexture("data/doorpostside1.raw", 64, 1024)), 64, 1024);	
	tp.CreateTexture(DOOR_POST_LIB, ImagePtr(tp.LoadTexture("data/doorPostLibSide.raw", 512, 64)), 512, 64);	
	tp.CreateTexture(GLASS_BOARD, ImagePtr(tp.LoadTexture("data/glassboard.raw", 512, 256)), 512, 256);	
	tp.CreateTexture(GLASS_BOARD_2, ImagePtr(tp.LoadTexture("data/glassboard2.raw", 512, 256)), 512, 256);	
	tp.CreateTexture(GLASS_BOARD_3, ImagePtr(tp.LoadTexture("data/glassboard3.raw", 512, 256)), 512, 256);	
	tp.CreateTexture(GLASS_B_SIDE, ImagePtr(tp.LoadTexture("data/glassboardSide.raw", 2, 16)), 2, 16);	
	tp.CreateTexture(GRASS, ImagePtr(tp.LoadTexture("data/grass.raw", 64, 64)), 64, 64);	
	tp.CreateTexture(GRASS_2, ImagePtr(tp.LoadTexture("data/grass2.raw", 64, 64)), 64, 64);	
	tp.CreateTexture(GRASS_HILL, ImagePtr(tp.LoadTexture("data/grass3.raw", 64, 64)), 64, 64);	
	tp.CreateTexture(GS_SIGN, ImagePtr(tp.LoadTexture("data/GSsign.raw", 256, 256)), 256, 256);	
	tp.CreateTexture(GS_SIGN_2, ImagePtr(tp.LoadTexture("data/GSsign2.raw", 256, 256)), 256, 256);	
	tp.CreateTexture(GS_SIGN_EDGE, ImagePtr(tp.LoadTexture("data/GSsignedge.raw", 256, 64)), 256, 64);	
	tp.CreateTexture(KBLT, ImagePtr(tp.LoadTexture("data/kbl.raw", 512, 256)), 512, 256);	
	tp.CreateTexture(KBLT_SIDE_1, ImagePtr(tp.LoadTexture("data/KBLTside1.raw", 2, 128)), 2, 128);	
	tp.CreateTexture(KBLT_SIDE_2, ImagePtr(tp.LoadTexture("data/KBLTside2.raw", 2, 2)), 2, 2);
	tp.CreateTexture(KBLT_EDGE_CORNER, ImagePtr(tp.LoadTexture("data/kblCorner.raw", 1, 1)), 1, 1);	
	tp.CreateTexture(KBLT_EDGE, ImagePtr(tp.LoadTexture("data/kbltEdge.raw", 16, 32)), 16, 32);	
	tp.CreateTexture(KBLT_EDGE_2, ImagePtr(tp.LoadTexture("data/kbltEdge2.raw", 32, 16)), 32, 16);	
	tp.CreateTexture(LIGHT, ImagePtr(tp.LoadTexture("data/light.raw", 256, 64)), 256, 64);	
	tp.CreateTexture(LIGHT_SUPPORT, ImagePtr(tp.LoadTexture("data/lightsupport.raw", 8, 2)), 8, 2);	
	tp.CreateTexture(LIGHT_SUPPORT_2, ImagePtr(tp.LoadTexture("data/lightsupport2.raw", 2, 8)), 2, 8);	
	tp.CreateTexture(MACHINE_SIDES, ImagePtr(tp.LoadTexture("data/machineSides.raw", 1, 1)), 1, 1);	
	tp.CreateTexture(MACHINE_SIDES_2, ImagePtr(tp.LoadTexture("data/machineSides2.raw", 1, 1)), 1, 1);	
	tp.CreateTexture(MAIN_POST, ImagePtr(tp.LoadTexture("data/mainPost.raw",  128, 256)), 128, 256);
	tp.CreateTexture(MAIN_POST_2, ImagePtr(tp.LoadTexture("data/mainPost2.raw", 256, 128)), 256, 128);	
	tp.CreateTexture(MAP_2, ImagePtr(tp.LoadTexture("data/map2.raw", 256, 512)), 256, 512);	
	tp.CreateTexture(NEXUS_SIGN, ImagePtr(tp.LoadTexture("data/nexus.raw", 512, 256)), 512, 256);	
	tp.CreateTexture(NEXUS_SIDE, ImagePtr(tp.LoadTexture("data/nexusSide.raw", 2, 16)), 2, 16);	
	tp.CreateTexture(NO_EXIT, ImagePtr(tp.LoadTexture("data/noexit.raw", 256, 64)), 256, 64);	
	tp.CreateTexture(NO_SMOKE_SIGN, ImagePtr(tp.LoadTexture("data/nosmokesign.raw", 256, 128)), 256, 128);	
	tp.CreateTexture(PAVEMENT, ImagePtr(tp.LoadTexture("data/pavement.raw", 128, 64)), 128, 64);	
	tp.CreateTexture(PAVEMENT_16, ImagePtr(tp.LoadTexture("data/pavement16.raw", 32, 64)), 32, 64);	
	tp.CreateTexture(PAVEMENT_FLIP, ImagePtr(tp.LoadTexture("data/pavementflip.raw", 128, 64)), 128, 64);	
	tp.CreateTexture(PAVEMENT_CORNER_1, ImagePtr(tp.LoadTexture("data/pavementcorner1.raw", 128, 128)), 128, 128);	
	tp.CreateTexture(PAVEMENT_CORNER_2, ImagePtr(tp.LoadTexture("data/pavementcorner2.raw", 128, 128)), 128, 128);	
	tp.CreateTexture(PAVEMENT_TOP, ImagePtr(tp.LoadTexture("data/pavementtop.raw", 64, 128)), 64, 128);	
	tp.CreateTexture(PAVEMENT_TOP_FLIP, ImagePtr(tp.LoadTexture("data/pavementtopflip.raw", 64, 128)), 64, 128);	
	tp.CreateTexture(PAVEMENTSIDE_LEFT, ImagePtr(tp.LoadTexture("data/pavementSide.raw", 64, 64)), 64, 64);	
	tp.CreateTexture(PAVEMENTSIDE_RIGHT, ImagePtr(tp.LoadTexture("data/pavementSide2.raw", 64, 64)), 64, 64);	
	tp.CreateTexture(PAVEMENTSIDE_TOP, ImagePtr(tp.LoadTexture("data/pavementSide3.raw", 64, 64)), 64, 64);
	tp.CreateTexture(TELEPHONE_FRONT, ImagePtr(tp.LoadTexture("data/phonefront.raw", 512, 32)), 512, 32);
	tp.CreateTexture(TELEPHONE_SIDE_1, ImagePtr(tp.LoadTexture("data/phoneside1.raw", 32, 256)), 32, 256);		
	tp.CreateTexture(TELEPHONE_FRONT_2, ImagePtr(tp.LoadTexture("data/phonefront2.raw", 512, 16)), 512, 16);	
	tp.CreateTexture(TELEPHONE_MAIN_SIDE, ImagePtr(tp.LoadTexture("data/phonemainside.raw", 512, 256)), 512, 256);
	tp.CreateTexture(TELEPHONE_TOP_1, ImagePtr(tp.LoadTexture("data/phonetop1.raw", 512, 128)), 512, 128);	
	tp.CreateTexture(TELEPHONE_SIDE_2, ImagePtr(tp.LoadTexture("data/phoneside2.raw", 16, 256)), 16, 256);	
	tp.CreateTexture(TELEPHONE_TOP_2, ImagePtr(tp.LoadTexture("data/phonetop2.raw", 128, 256)), 128, 256);	
	tp.CreateTexture(TELEPHONE_BOTTOM, ImagePtr(tp.LoadTexture("data/phonebottom.raw", 512, 128)), 512, 128);	
	tp.CreateTexture(TELEPHONE_FILL, ImagePtr(tp.LoadTexture("data/phoneFill.raw", 2, 2)), 2, 2);	
	tp.CreateTexture(TELEPHONE_FRONT_3, ImagePtr(tp.LoadTexture("data/phonefront3.raw", 16, 512)), 16, 512);	
	tp.CreateTexture(PSC_SIGN, ImagePtr(tp.LoadTexture("data/PSCsign.raw", 256, 128)), 256, 128);
	tp.CreateTexture(PSC_SIGN_2, ImagePtr(tp.LoadTexture("data/PSCsign2.raw", 256, 128)), 256, 128);	
	tp.CreateTexture(PURPLE_POST, ImagePtr(tp.LoadTexture("data/purplepost.raw", 64, 128)), 64 ,128);
	tp.CreateTexture(PURPLE_POSTSIDE, ImagePtr(tp.LoadTexture("data/purplepostside.raw", 128, 64)), 128, 64);	
	tp.CreateTexture(RED_POST, ImagePtr(tp.LoadTexture("data/redpost.raw", 64, 128)), 64 ,128);
	tp.CreateTexture(RED_POSTSIDE, ImagePtr(tp.LoadTexture("data/redpostside.raw", 64, 64)), 64, 64);	
	tp.CreateTexture(ROOF_BEAM_1, ImagePtr(tp.LoadTexture("data/roofbeam1.raw", 128, 32)), 128, 32);	
	tp.CreateTexture(ROOF_BEAM_2, ImagePtr(tp.LoadTexture("data/roofbeam2.raw", 32, 128)), 32, 128);	
	tp.CreateTexture(ROOF_BEAM_3, ImagePtr(tp.LoadTexture("data/roofbeam3.raw", 64, 128)), 64, 128);	
	tp.CreateTexture(ROOF_BEAM_3_TOP, ImagePtr(tp.LoadTexture("data/roofbeam3top.raw", 128, 128)), 128, 128);	
	tp.CreateTexture(ROOF_BEAM_4, ImagePtr(tp.LoadTexture("data/roofbeam4.raw", 128, 64)), 128, 64);	
	tp.CreateTexture(ROOF_PLANKS, ImagePtr(tp.LoadTexture("data/roofPlanks.raw", 128, 128)), 128, 128);	
	tp.CreateTexture(ROOF_PLANKS_2, ImagePtr(tp.LoadTexture("data/roofPlanks2.raw", 128, 128)), 128, 128);	
	tp.CreateTexture(ROOF_TOP, ImagePtr(tp.LoadTexture("data/rooftop.raw", 2, 128)), 2, 128);	
	tp.CreateTexture(ROOF_TOP_LIB, ImagePtr(tp.LoadTexture("data/rooftoplib.raw", 128, 2)), 128, 2);	
	tp.CreateTexture(RUSTY_MAN, ImagePtr(tp.LoadTexture("data/rustyman.raw", 256, 1024)), 256, 1024);	
	tp.CreateTexture(SECURITY_SIGN, ImagePtr(tp.LoadTexture("data/securitysign1.raw", 128, 128)), 128, 128);	
	tp.CreateTexture(SECURITY_SIGN_2, ImagePtr(tp.LoadTexture("data/securitysign2.raw", 128, 128)), 128, 128);	
	tp.CreateTexture(SIGN_1, ImagePtr(tp.LoadTexture("data/sign1.raw", 512, 256)), 512, 256);
	tp.CreateTexture(SIGN_1_SIDE_1, ImagePtr(tp.LoadTexture("data/sign1side1.raw", 16, 512)), 16, 512);	
	tp.CreateTexture(SIGN_1_SIDE_2, ImagePtr(tp.LoadTexture("data/sign1side2.raw", 16, 512)), 16, 512);	
	tp.CreateTexture(SIGN_2, ImagePtr(tp.LoadTexture("data/sign2.raw", 512, 512)), 512, 512);	
	tp.CreateTexture(SIGN_2_SIDE, ImagePtr(tp.LoadTexture("data/sign2side.raw", 512, 16)), 512, 16);	
	tp.CreateTexture(STA_TRAVEL, ImagePtr(tp.LoadTexture("data/statravel.raw", 256, 256)), 256, 256);	
	tp.CreateTexture(STA_TRAVEL_EDGE, ImagePtr(tp.LoadTexture("data/statraveledge.raw", 256, 64)), 256, 64);	
	tp.CreateTexture(STA_TRAVEL_BRACKET, ImagePtr(tp.LoadTexture("data/statravelbracket.raw", 16, 2)), 16, 2);	
	tp.CreateTexture(STA_TRAVEL_2, ImagePtr(tp.LoadTexture("data/statravel2.raw", 256, 256)), 256, 256);	
	tp.CreateTexture(STA_TRAVEL_BOTTOM, ImagePtr(tp.LoadTexture("data/statravelbottom.raw", 2, 64)), 2, 64);	
	tp.CreateTexture(WALL_BRICK_STEPS, ImagePtr(tp.LoadTexture("data/stepbricks.raw", 128, 128)), 128, 128);	
	tp.CreateTexture(WALL_BRICK_STEPS_COVER, ImagePtr(tp.LoadTexture("data/stepbrickscover.raw", 64, 128)), 64, 128);	
	tp.CreateTexture(WALL_BRICK_STEPS_EDGE, ImagePtr(tp.LoadTexture("data/stepbricksedge.raw", 64, 32)), 64, 32);
	tp.CreateTexture(WALL_BRICK_STEPS_EDGE_2, ImagePtr(tp.LoadTexture("data/stepbricksedge2.raw", 64, 64)), 64, 64);	
	tp.CreateTexture(WALL_BRICK_STEPS_TOP, ImagePtr(tp.LoadTexture("data/stepbrickstop.raw", 32, 128)), 32, 128);	
	tp.CreateTexture(STEPS_LIBRARY, ImagePtr(tp.LoadTexture("data/stepslibrary.raw", 128, 1024)), 128, 1024);
	tp.CreateTexture(STEPS_LIBRARY_TOP, ImagePtr(tp.LoadTexture("data/stepslibrarytop.raw", 256, 1024)), 256, 1024);	
	tp.CreateTexture(STEP_PAVING_1, ImagePtr(tp.LoadTexture("data/steppaving1.raw", 1024, 512)), 1024, 512);	
	tp.CreateTexture(STEP_EDGE, ImagePtr(tp.LoadTexture("data/steppavingedge.raw", 64, 64)), 64, 64);	
	tp.CreateTexture(SWEET_MACHINE, ImagePtr(tp.LoadTexture("data/sweetMachine.raw", 256, 256)), 256, 256);	
	tp.CreateTexture(TELEPHONE_BACK, ImagePtr(tp.LoadTexture("data/telephoneback.raw", 512, 512)), 512, 512);	
	tp.CreateTexture(TICKET_COUNTER_EDGE_2, ImagePtr(tp.LoadTexture("data/ticketcounteredge2.raw", 64, 64)), 64, 64);	
	tp.CreateTexture(TICKET_COUNTER_EDGE_3, ImagePtr(tp.LoadTexture("data/ticketcounteredge3.raw", 64, 64)), 64, 64);	
	tp.CreateTexture(TICKET_COUNTER_TOP, ImagePtr(tp.LoadTexture("data/ticketcountertop.raw", 128, 256)), 128, 256);	
	tp.CreateTexture(TICKET_LEDGE_EDGE, ImagePtr(tp.LoadTexture("data/ticketledgeedge.raw", 2, 32)), 2, 32);	
	tp.CreateTexture(TICKET_LEDGE, ImagePtr(tp.LoadTexture("data/ticketledge.raw", 2, 128)), 2, 128);	
	tp.CreateTexture(TICKET_LEDGE_EDGE_2, ImagePtr(tp.LoadTexture("data/ticketledgeedge2.raw", 32, 2)), 32, 2);	
	tp.CreateTexture(TICKET_COUNTER_EDGE, ImagePtr(tp.LoadTexture("data/ticketcounteredge.raw", 128, 32)), 128, 32);	
	tp.CreateTexture(TOILET_DOOR_TOP, ImagePtr(tp.LoadTexture("data/toiletdoortop.raw", 32, 256)), 32, 256);	
	tp.CreateTexture(TOILET_MEN, ImagePtr(tp.LoadTexture("data/toiletmen.raw", 128, 128)), 128, 128);	
	tp.CreateTexture(TOILET_WOMEN, ImagePtr(tp.LoadTexture("data/toiletwomen.raw", 128, 128)), 128, 128);	
	tp.CreateTexture(WALL_GAP_1, ImagePtr(tp.LoadTexture("data/wallgap1.raw", 8, 128)), 8, 128);	
	tp.CreateTexture(WINDOWLEDGE_CHANC_FRONT, ImagePtr(tp.LoadTexture("data/windowledge1.raw", 32, 1024)), 32, 1024);	
	tp.CreateTexture(WINDOWLEDGE_CHANC_TOP, ImagePtr(tp.LoadTexture("data/windowledge2.raw", 128, 1024)), 128, 1024);	
	tp.CreateTexture(WINDOW_LEDGE_END_1, ImagePtr(tp.LoadTexture("data/windowledgeend.raw", 32, 64)), 32, 64);	
	tp.CreateTexture(WINDOW_LEDGE_END_2, ImagePtr(tp.LoadTexture("data/windowledgeend2.raw", 64, 64)), 64, 64);	
	tp.CreateTexture(WINDOWLEDGE_LIB_A, ImagePtr(tp.LoadTexture("data/windowledgeLibfa.raw", 1024, 32)), 1024, 32);	
	tp.CreateTexture(WINDOWLEDGE_LIB_B, ImagePtr(tp.LoadTexture("data/windowledgeLibfb.raw", 1024, 32)), 1024, 32);	
	tp.CreateTexture(WINDOWLEDGE_LIB_TOP_A, ImagePtr(tp.LoadTexture("data/windowledgeLibta.raw", 1024, 128)), 1024, 128);	
	tp.CreateTexture(WINDOWLEDGE_LIB_TOP_B, ImagePtr(tp.LoadTexture("data/windowledgeLibtb.raw", 1024, 128)), 1024, 128);	
	tp.CreateTexture(WINDOWLEDGE_PS_FRONT, ImagePtr(tp.LoadTexture("data/windowledgeUS1.raw", 32, 1024)), 32, 1024);	
	tp.CreateTexture(WINDOWLEDGE_PS_TOP, ImagePtr(tp.LoadTexture("data/windowledgeUS2.raw", 128, 1024)), 128, 1024);	
	tp.CreateTexture(WINDOWLEDGE_PS_BOTT, ImagePtr(tp.LoadTexture("data/windowledgeUS3.raw", 128, 1024)), 128, 1024);	
	tp.CreateTexture(WINDOWPOST_CHANC_FRONT, ImagePtr(tp.LoadTexture("data/windowpost1.raw", 1024, 128)), 1024, 128);	
	tp.CreateTexture(WINDOWPOST_PHYSSCI_FRONT, ImagePtr(tp.LoadTexture("data/windowpostsmall.raw", 512, 128)), 512, 128);	
	tp.CreateTexture(WINDOWPOST_PHYSSCI_RIGHT, ImagePtr(tp.LoadTexture("data/windowpostsmallside2.raw", 64, 512)), 64, 512);	
	tp.CreateTexture(WINDOWPOST_PHYSSCI_LEFT, ImagePtr(tp.LoadTexture("data/windowpostsmallside.raw", 64, 512)), 64, 512);
	tp.CreateTexture(WINDOWPOST_LIB_FRONT, ImagePtr(tp.LoadTexture("data/windowpostLib.raw", 128, 512)), 128, 512);	
	tp.CreateTexture(WINDOWPOST_LIB_LEFT, ImagePtr(tp.LoadTexture("data/windowpostLibside1.raw", 512, 64)), 512, 64);	
	tp.CreateTexture(WINDOWPOST_LIB_RIGHT, ImagePtr(tp.LoadTexture("data/windowpostLibside2.raw", 512, 64)), 512, 64);	
	tp.CreateTexture(WINDOWPOST_CHANC_RIGHT, ImagePtr(tp.LoadTexture("data/windowposthalf1.raw", 64, 1024)), 64, 1024);
	tp.CreateTexture(WINDOWPOST_CHANC_LEFT, ImagePtr(tp.LoadTexture("data/windowposthalf2.raw", 64, 1024)), 64, 1024);
	// Larger Textures
	tp.CreateTexture(STEP_WINDOW, ImagePtr(tp.LoadTexture("data/windows/stepwindow.raw", 128, 256)), 128, 256);
	tp.CreateTexture(CHANC_DOOR_1, ImagePtr(tp.LoadTexture("data/windows/chancDoor1.raw", 256, 256)), 256, 256);
	tp.CreateTexture(CHANC_DOOR_2, ImagePtr(tp.LoadTexture("data/windows/chancDoor2.raw", 256, 256)), 256, 256);
	tp.CreateTexture(ENTRANCE, ImagePtr(tp.LoadTexture("data/windows/entrance.raw", 512, 256)), 512, 256);
	tp.CreateTexture(ENTRANCE_2, ImagePtr(tp.LoadTexture("data/windows/entrance2.raw",512, 512)), 512, 512);
	tp.CreateTexture(EXIT_EAST, ImagePtr(tp.LoadTexture("data/windows/exiteast.raw", 512, 512)), 512, 512);
	tp.CreateTexture(EXIT_WEST, ImagePtr(tp.LoadTexture("data/windows/exitwest.raw", 256, 512)), 256, 512);
	tp.CreateTexture(WINDOW_1, ImagePtr(tp.LoadTexture("data/windows/window1.raw", 256, 256)), 256, 256);
	tp.CreateTexture(WINDOW_1B, ImagePtr(tp.LoadTexture("data/windows/window1b.raw", 256, 256)), 256, 256);
	tp.CreateTexture(WINDOW_2, ImagePtr(tp.LoadTexture("data/windows/window2.raw", 256, 256)), 256, 256);
	tp.CreateTexture(WINDOW_2B, ImagePtr(tp.LoadTexture("data/windows/window2b.raw", 256, 256)), 256, 256);
	tp.CreateTexture(WINDOW_2C, ImagePtr(tp.LoadTexture("data/windows/window2c.raw", 256, 256)), 256, 256);
	tp.CreateTexture(WINDOW_2D, ImagePtr(tp.LoadTexture("data/windows/window2d.raw", 256, 256)), 256, 256);
	tp.CreateTexture(WINDOW_2E, ImagePtr(tp.LoadTexture("data/windows/window2e.raw", 256, 256)), 256, 256);
	tp.CreateTexture(WINDOW_2US, ImagePtr(tp.LoadTexture("data/windows/window2us.raw", 256, 256)), 256, 256);
	tp.CreateTexture(WINDOW_2USB, ImagePtr(tp.LoadTexture("data/windows/window2usb.raw", 256, 256)), 256, 256);
	tp.CreateTexture(WINDOW_3, ImagePtr(tp.LoadTexture("data/windows/window3.raw", 256, 256)), 256, 256);
	tp.CreateTexture(WINDOW_3B, ImagePtr(tp.LoadTexture("data/windows/window3b.raw", 256, 256)), 256, 256);
	tp.CreateTexture(WINDOW_4, ImagePtr(tp.LoadTexture("data/windows/window4.raw", 128, 256)), 128, 256);
	tp.CreateTexture(WINDOW_5, ImagePtr(tp.LoadTexture("data/windows/window5.raw", 128, 256)), 128, 256);
	tp.CreateTexture(WINDOW_6, ImagePtr(tp.LoadTexture("data/windows/window6.raw", 256, 256)), 256, 256);
	tp.CreateTexture(WINDOW_7, ImagePtr(tp.LoadTexture("data/windows/window7.raw", 256, 256)), 256, 256);
	tp.CreateTexture(WINDOW_8, ImagePtr(tp.LoadTexture("data/windows/window8.raw", 256, 256)), 256, 256);
	tp.CreateTexture(WINDOW_9, ImagePtr(tp.LoadTexture("data/windows/window9.raw", 256, 256)), 256, 256);
	tp.CreateTexture(WINDOW_10, ImagePtr(tp.LoadTexture("data/windows/window10.raw", 256, 256)), 256, 256);
	tp.CreateTexture(WINDOW_11, ImagePtr(tp.LoadTexture("data/windows/window11.raw", 256, 256)), 256, 256);
	tp.CreateTexture(WINDOW_12, ImagePtr(tp.LoadTexture("data/windows/window12.raw", 256, 256)), 256, 256);
	tp.CreateTexture(WINDOW_13, ImagePtr(tp.LoadTexture("data/windows/window13.raw", 256, 256)), 256, 256);
	tp.CreateTexture(WINDOW_14, ImagePtr(tp.LoadTexture("data/windows/window14.raw", 256, 128)), 256, 128);
	tp.CreateTexture(WINDOW_14B, ImagePtr(tp.LoadTexture("data/windows/window14b.raw", 256, 128)), 256, 128);
	tp.CreateTexture(WINDOW_15, ImagePtr(tp.LoadTexture("data/windows/window15.raw", 256, 256)), 256, 256);
	tp.CreateTexture(WINDOW_16, ImagePtr(tp.LoadTexture("data/windows/window16.raw", 128, 256)), 128, 256);
	tp.CreateTexture(WINDOW_17, ImagePtr(tp.LoadTexture("data/windows/window17.raw", 128, 256)), 128, 256);
	tp.CreateTexture(WINDOW_LIB_1, ImagePtr(tp.LoadTexture("data/windows/windowLib1.raw", 256, 128)), 256, 128);
	tp.CreateTexture(WINDOW_LIB_1A, ImagePtr(tp.LoadTexture("data/windows/windowLib1a.raw", 256, 128)), 256, 128);
	tp.CreateTexture(WINDOW_LIB_1B, ImagePtr(tp.LoadTexture("data/windows/windowLib1b.raw", 256, 128)), 256, 128);
	tp.CreateTexture(WINDOW_LIB_1C, ImagePtr(tp.LoadTexture("data/windows/windowLib1c.raw", 256, 128)), 256, 128);
	tp.CreateTexture(WINDOW_LIB_US_A, ImagePtr(tp.LoadTexture("data/windows/windowLibUSa.raw", 256, 128)), 256, 128);
	tp.CreateTexture(WINDOW_LIB_US_B, ImagePtr(tp.LoadTexture("data/windows/windowLibUSb.raw", 256, 128)), 256, 128);
...
//--------------------------------------------------------------------------------------
void ShayWorld::DisplayChancPosts ()
{
...
void ShayWorld::DisplayDoorPosts ()
{
	// Door Posts Chanc
...
//--------------------------------------------------------------------------------------
void ShayWorld::DisplayAboveWindowBlock ()
{
...
void ShayWorld::DrawAboveWindowBlock ()
{
...
//--------------------------------------------------------------------------------------
void ShayWorld::DisplayPurplePosts ()
{
...
void ShayWorld::DrawPurplePosts ()
{
...
void ShayWorld::DisplayRedPosts ()
{
...
// Display Main Posts
//--------------------------------------------------------------------------------------
void ShayWorld::DisplayMainPosts ()
{
	step = 0.0;
	stepLength = 0.0;
	step2 = 0.0;
...
//--------------------------------------------------------------------------------------
void ShayWorld::DisplayPhysSciPosts ()
{
	step = 0.0;
	for (int i = 0; i < 16; i++)
	{
...
void ShayWorld::DrawPhysSciPosts ()
{
	// WINDOWPOST_PS
...
//--------------------------------------------------------------------------------------
void ShayWorld::DisplayDoorPaving ()
{
...
//--------------------------------------------------------------------------------------
void ShayWorld::DisplayLibraryPosts ()
{
	stepLength = 0.0;
	for (int j = 0; j < 2; j++)
	{
...
void ShayWorld::DrawLibraryPosts ()
{
	// WINDOWPOST_LIB_FRONT
...
void ShayWorld::DrawPavement ()
{
	...
	// corridoor floor
	tp.CreateDisplayList (XZ, 1149, 128.0, 64.0, -5000, 9136.0, 20240.0, 7608.0 / 128.0, 760.0 / 64.0);
}
...
void ShayWorld::DisplayBricks ()
{
	...
	for (int i = 1000; i < 1050; i ++) glCallList(i);
	...
	for (int i = 1050; i < 1100; i ++) glCallList(i);
...
void ShayWorld::DrawBricks ()
{
	...
	tp.CreateDisplayList (YZ, 122, 128.0, 128.0, 2608.0, 10000.0, 21000.0, 7.0, 157.58);	// upper long downstairs panel
	tp.CreateDisplayList (YZ, 1000, 128.0, 128.0, 2608.0, 10000.0, 10000.0, 7.0, 80.0);		// lower long downstairs panel

	tp.CreateDisplayList (XY, 1050, 128.0, 128.0, -5000.0, 9136.0, 20240.0, 7608.0 / 128.0, 13.75);	// corridoor left panel
	tp.CreateDisplayList (XY, 1051, 128.0, 128.0, -5000.0, 9136.0, 21000.0, 7608.0 / 128.0, 13.75);	// corridoor right panel
	//tp.CreateDisplayList (YZ, 1001, 128.0, 128.0, -5000.0, 9136.0, 20240.0, 896.0 / 128.0, 760.0 / 128.0);	// corridoor back panel
...
void ShayWorld::DrawRoof()
{
	...
	// corridoor roof
	tp.CreateDisplayList (XZ, 1148, 128.0, 64.0, -5000, 10032.0, 20240.0, 5052.0 / 128.0, 760.0 / 64.0);
}

// --------------------------------------------------------------------------------------
//  Creates Angled Roof Beams
// --------------------------------------------------------------------------------------
void ShayWorld::DrawAngledRoofBeam (int listNo, GLfloat x, GLfloat y, GLfloat z, GLfloat beamSize)
{
...
void ShayWorld::DrawAngledRoofBeam2 (int listNo, GLfloat x, GLfloat y, GLfloat z, GLfloat beamSize)
{
	glNewList(listNo, GL_COMPILE);
	glBegin(GL_QUADS);
	glTexCoord2f(0.0, 0.0);
...
// Display Steps
//--------------------------------------------------------------------------------------
void ShayWorld::DisplayEntranceSteps ()
{
...
void ShayWorld::DrawEntranceSteps ()
{	
	step = 10000.0;
	stepLength = 9808.0;
	...
	step = 10000.0;
	stepLength = 2466.0;
	for (int i = 1100; i < 1118 ; i ++)
	{
		tp.CreateDisplayList (XZ, i, 128.0, 64.0, stepLength, step, 20240.0, 142.0 / 128.0, 760.0 / 64.0);
		tp.CreateDisplayList (YZ, i + 18, 128.0, 64.0, stepLength, step - 48.0, 20240.0, 48.0 / 128.0, 760.0 / 64.0);
		step -= 48.0;
		stepLength -= 142.0;
	}
...
void ShayWorld::DisplayLargerTextures ()
{
...
void ShayWorld::DrawLargerTextures ()
{
	// CHANC
...
void ShayWorld::DisplayGrass ()
{
	glBindTexture(GL_TEXTURE_2D, tp.GetTexture(GRASS));
	glCallList(79);
	glCallList(111);
	for (int i = 460; i < 478; i++) glCallList(i);
...
void ShayWorld::DrawGrass ()
{
	...
	tp.CreateAngledPolygon(1147, 128.0, 64.0,  52.0,     2608.0,   2608.0,   52.0,
		10032.0,  10896.0,  10896.0,  10032.0,
		20240.0,  20240.0,  21000.0,  21000.0, 1, 1);
}

// --------------------------------------------------------------------------------------
// Display Light Fittings
// --------------------------------------------------------------------------------------
void ShayWorld::DisplayLights ()
{
	// Light Fitting
...
void ShayWorld::DisplayCylinders ()
{
	// drainpipe
...
// --------------------------------------------------------------------------------------
void ShayWorld::DisplayStepBricks ()
{
	step = 0.0;
...
//--------------------------------------------------------------------------------------
void ShayWorld::CreateTextureList()
{
...
//--------------------------------------------------------------------------------------
void ShayWorld::IncrementFrameCount()
{
